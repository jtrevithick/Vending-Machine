//James Trevithick											Trevithick2@cnm.edu						TrevithickP6
...
int To_string(String^ source, string &target)
{
	...
	size_t i;		//number of characters converted
					...
					If wcstombs successfully converts the multibyte string, it returns
					the number of bytes written into the multibyte output string,
					excluding the terminating NULL, if any.  If the conversion fails,
					it returns an error code*/
					//ch = char* dest destination or target
					//wch = const wchar_t* src  or size of  source
					//len = size_t max
